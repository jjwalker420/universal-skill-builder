...
- Model: "Here are the stories. Summarize the two most relevant to LYTVOX."
...
When to use: morning-briefing, eod-sync, any skill that monitors a source over time.
...
When to use: morning-briefing (4 sources), eod-sync (5 sources), any skill with 3+ parallel fetches.
...
## JJ's Retrofit Candidates

These existing skills are Type A and currently run without scripts.
Each is a candidate for a script audit:

| Skill | Issue | Pattern to apply |
|-------|-------|-----------------|
| morning-briefing | 4 sequential MCP calls through model | 7 (batch), 4 (schema) |
| eod-sync | 5 sequential MCP calls through model | 7 (batch), 4 (schema) |
| financial-advisor | Copilot data retrieval through model | 4 (schema), 3 (heavy lifter) |
| meeting-review | Full transcript loaded into context | 3 (heavy lifter), 4 (schema) |
| github-security-audit | Clones and scans inline | 5 (stop conditions), 6 (incremental) |

Retrofitting these is post-v1. Document now, execute after templates are proven.
